...
// recordingExecer captures each forwarded argv tuple. It only exists so the
// internal tests can observe what commandEnvExecer hands to the wrapped
// execer, without depending on the external test-package fakeExecer.
...
// This is the regression test for PLAT-261 / SEC-101: an earlier version
// embedded each argument into a `sh -c` script via fmt.Sprintf("%q", arg),
// which allowed values such as Docker container labels to trigger shell
// expansion of $(...), backticks and $VAR.
...
func TestCommandEnvExecer_SetsEnvAndDir(t *testing.T) {
	...
	c := e.CommandContext(context.Background(), "docker", "ps")
...
func TestCommandEnvExecer_CommandEnvError(t *testing.T) {
	...
	c := e.CommandContext(context.Background(), "docker", "ps", "--all")
	require.Len(t, rec.commands, 1)
...
func TestLookPathInEnv(t *testing.T) {
	...
	dir1 := t.TempDir()
	dir2 := t.TempDir()
	// dir1 contains: a non-executable "tool", a directory named "alsotool".
	// dir2 contains: an executable "tool" and an executable "alsotool".
	require.NoError(t, os.WriteFile(filepath.Join(dir1, "tool"), []byte("not exec"), 0o600))
	require.NoError(t, os.Mkdir(filepath.Join(dir1, "alsotool"), 0o755))
	require.NoError(t, os.WriteFile(filepath.Join(dir2, "tool"), []byte("#!/bin/sh\nexit 0\n"), 0o600))
	require.NoError(t, os.Chmod(filepath.Join(dir2, "tool"), 0o755))
	require.NoError(t, os.WriteFile(filepath.Join(dir2, "alsotool"), []byte("#!/bin/sh\nexit 0\n"), 0o600))
	require.NoError(t, os.Chmod(filepath.Join(dir2, "alsotool"), 0o755))
	...
	tests := []struct {
		...
		},
		{
			...
			want:    filepath.Join(dir2, "tool"),
		},
		{
			name:    "SkipsNonExecutable",
			nameArg: "tool",
			env:     []string{"PATH=" + dir1},
			wantErr: true,
		},
		{
			name:    "SkipsDirectory",
			nameArg: "alsotool",
			env:     []string{"PATH=" + dir1 + string(os.PathListSeparator) + dir2},
			want:    filepath.Join(dir2, "alsotool"),
		},
		{
			name:    "MissingPATH",
			nameArg: "tool",
		...
		},
		{
			...
			nameArg: "tool",
			env:     []string{"PATH=/nonexistent", "PATH=" + dir2},
			want:    filepath.Join(dir2, "tool"),
		},
		{
			name:    "EmptyName",
