...
// New constructs a chat worker. The worker is idle until Start is called.
...
func (w *chatWorker) acquireCandidate(
	...
	runnerID := uuid.New()
	machine := chatstate.NewChatMachine(w.opts.Store, w.opts.Pubsub, chatID, chatstate.Options{})
...
func (w *chatWorker) abandonAcquiredChat(ctx context.Context, workerID uuid.UUID, runnerID uuid.UUID, chatID uuid.UUID) error {
	...
	defer cancel()
	machine := chatstate.NewChatMachine(w.opts.Store, w.opts.Pubsub, chatID, chatstate.Options{})
	err := machine.Update(cleanupCtx, func(tx *chatstate.Tx) error {
		chat, err := tx.Store().GetChatByID(cleanupCtx, chatID)
		if errors.Is(err, sql.ErrNoRows) {
			return errSkipAcquire
		}
		if err != nil {
