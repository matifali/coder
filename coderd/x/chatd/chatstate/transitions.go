...
func CreateChat(
	...
	err := store.InTx(func(store database.Store) error {
		chat, ierr := store.InsertChat(ctx, database.InsertChatParams{
		...
		if input.LastInjectedContext.Valid {
			if _, ierr := store.UpdateChatLastInjectedContext(ctx, database.UpdateChatLastInjectedContextParams{
				ID:                  chat.ID,
				LastInjectedContext: input.LastInjectedContext,
			}); ierr != nil {
				return xerrors.Errorf("set last injected context: %w", ierr)
			}
		}
		...
		if ClassifyExecutionState(refreshed, false, true).IsRunnable() {
			if perr := buffer.Publish(
				...
				return xerrors.Errorf("buffer ownership hint: %w", perr)
			}
		}
		return nil
	}, nil)
