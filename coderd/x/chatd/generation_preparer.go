...
func (server *Server) prepareGeneration(
	...
	if advisorRuntime != nil {
		var publishAdviceDelta func(string, string)
		var publishAdviceReset func(string)
		if input.PublishMessagePart != nil {
			publishAdviceDelta = func(toolCallID string, delta string) {
				if toolCallID == "" || delta == "" {
					return
				}
				input.PublishMessagePart(codersdk.ChatMessageRoleTool, codersdk.ChatMessagePart{
		...
		tools = append(tools, chatadvisor.Tool(chatadvisor.ToolOptions{
			...
			PublishAdviceDelta: publishAdviceDelta,
			PublishAdviceReset: publishAdviceReset,
		}))
	}
	...
	if debugEnabled {
		...
		var finishDebugRun func(error, any)
