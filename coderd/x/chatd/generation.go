...
type generationPrepared struct {
	// Chat may be set when metadata side effects, such as resolving and
	// persisting a workspace agent binding, have mutated the chat row since
	// the worker last loaded it. When zero, the worker's locked chat
	// snapshot is used for generation decisions and actions.
	Chat     database.Chat
	Messages []database.ChatMessage
...
func (s *taskStarter) StartGeneration(ctx context.Context, input chatWorkerTaskStartInput) error {
	...
	for {
		...
		decisionChat := locked
		if prepared.Chat.ID != uuid.Nil {
			decisionChat = prepared.Chat
		}
		...
		case generationActionEnterRequiresAction:
			cleanup()
			return s.enterRequiresAction(ctx, machine, input, decision)
		...
		case generationActionGenerateAssistant:
			actionErr = s.generateAssistant(ctx, machine, input, prepareInput, decision)
		case generationActionExecuteLocalTools:
			actionErr = s.executeLocalTools(ctx, machine, input, prepareInput, decision)
		case generationActionCompact:
			actionErr = s.generateCompaction(ctx, machine, input, prepareInput)
		case generationActionPersistWorkspaceContext:
			actionErr = s.persistWorkspaceContext(ctx, machine, input, decisionChat)
		default:
...
func (s *taskStarter) generateAssistant(
	...
	attempt, key, publish, closeEpisode, err := s.beginGenerationAttempt(ctx, machine, input)
	if err != nil {
		return err
	}
	defer closeEpisode()
	prepareInput.PublishMessagePart = publish
	prepared, err := s.server.prepareGeneration(ctx, prepareInput)
	if err != nil {
		return err
	}
	if prepared.Cleanup != nil {
	...
	outcome, err := chatloop.GenerateAssistant(ctx, chatloop.GenerateAssistantOptions{
...
func (s *taskStarter) persistWorkspaceContext(
	...
	if len(result.Messages) == 0 {
		// Builder reported no durable messages to commit (workspace or
...
func (s *taskStarter) finishGenerationTurn(
	...
	err := machine.Update(ctx, func(tx *chatstate.Tx) error {
		...
		if committed.ID == uuid.Nil {
			committed, err = tx.Store().GetChatByID(ctx, input.ChatID)
		} else if refreshed, refreshErr := tx.Store().GetChatByID(ctx, input.ChatID); refreshErr == nil {
			committed.LastTurnSummary = refreshed.LastTurnSummary
		} else {
			err = refreshErr
		}
		if err != nil {
