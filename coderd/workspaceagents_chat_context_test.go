...
func acquireAgentChatContextChat(ctx context.Context, t testing.TB, db database.Store, chatID uuid.UUID) database.Chat {
	...
	machine := chatstate.NewChatMachine(db, dbpubsub.NewInMemory(), chatID, chatstate.Options{})
	require.NoError(t, machine.Update(dbauthz.AsSystemRestricted(ctx), func(tx *chatstate.Tx) error {
