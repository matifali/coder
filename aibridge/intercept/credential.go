...
// failover across keys. The key in use evolves as the failover loop selects
// keys, so it is used via a pointer.
...
// centralized credential (e.g. Bedrock, which signs with AWS); only such
// requests fail over across keys.
