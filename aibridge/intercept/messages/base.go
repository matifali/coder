...
type interceptionBase struct {
	...
	// cred is the credential presented upstream for this interception: the
	// resolved authentication plus the key currently in use.
...
func (i *interceptionBase) newMessagesService(ctx context.Context, opts ...option.RequestOption) (anthropic.MessageService, error) {
	// Only BYOK sets its credential here. Centralized injects per-attempt in
	// the failover loop, and Bedrock signs via AWS below.
