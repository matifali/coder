...
import { RightPanelAddTabControl } from "../../RightPanel/RightPanelAddTabControl";
	...
	render: function CloseableTabs() {
		...
		return (
			<SidebarTabView
				...
				isExpanded={false}
				onToggleExpanded={() => {}}
				addTabControl={<RightPanelAddTabControl onNewTerminal={fn()} />}
			/>
		);
	},
...
export const AddTabControlDisabled: Story = {
	args: {
		tabs: [gitTab],
		addTabControl: <RightPanelAddTabControl disabled onNewTerminal={fn()} />,
	},
	play: async ({ canvasElement }) => {
		const canvas = within(canvasElement);
		expect(
